...
%Hacer un nodo que atienda un servicio llamado /navigation/a_star
%Responde si pudo o no calcular la trayectoria
%En el callback se debe hacer: crecer los obstáculos, calcular A_star, suavizar, en ese orden
%Una vez calculada la trayectoria, se debe publicar en un tópico de tipo path en /navigation/a_star_path
%Ajustar alpha y beta hasta obtener un suavizado satisfactorio

\subsection{Interfaz gráfica}
%Explicar la interfaz gráfica
%Poner un nuevo launch file que levante la interfaz gráfica
%Imagen de la interfaz gráfica.

%Ejemplo del resultado esperado con figura


\section{Evaluación}
\begin{itemize}
\item %El cálculo no debe tomar más de unos cuántos segundos. 
\item %Revisar que inicio y meta NO estén en el espacio ocupado.
...
\item %Las posiciones iniciales y finales sí se deben poder cambiar en runtime y se fijarán haciendo uso de la gui
\item %Cada cosa debe estar en métodos diferentes
\end{itemize}
