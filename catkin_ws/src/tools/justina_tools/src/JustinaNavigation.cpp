...
ros::ServiceClient JustinaNavigation::cltCalculatePath;
...
bool JustinaNavigation::setNodeHandle(ros::NodeHandle* nh)
{
    ...
    cltCalculatePath = nh->serviceClient<navig_msgs::CalculatePath>("/navigation/a_star");
...
void JustinaNavigation::getRobotPoseFromOdom(float& currentX, float& currentY, float& currentTheta)
{
	tf::StampedTransform transform;
	tf::Quaternion q;
...
bool JustinaNavigation::calcPathAStar(float startX, float startY, float goalX, float goalY, nav_msgs::Path& path)
{
    nav_msgs::GetMap srvGetMap;
    if(!cltGetMap.call(srvGetMap))
    {
        std::cout << "JustinaNavig.->Cannot get map for calculating path" << std::endl;
        return false;
    }
    ...
    bool success = JustinaNavigation::cltCalculatePath.call(srv);
